...
import { CardProductResponse } from "@/types/product";
import Link from "next/link";
import Image from "next/image"; // Khuyên dùng Image của Next.js để tối ưu hiệu năng

interface CardProductProps {
    product: CardProductResponse;
}
...
export const CardProduct = ({ product }: CardProductProps) => {
    return (
        <Link
            ...
            className="group cursor-pointer flex flex-col h-full bg-white rounded-2xl overflow-hidden border border-transparent hover:border-gray-100 hover:shadow-xl transition-all duration-300"
        >
            {/* Ảnh Thumbnail */}
            <div className="relative aspect-[4/3] overflow-hidden bg-gray-100">
                <Image
                    src={String(product.thumbnailUrl || 'https://via.placeholder.com/600x450')}
                    alt={String(product.title || "Product image")}
                    fill
                    sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"
                    className="object-cover w-full h-full group-hover:scale-110 transition-transform duration-500"
                    unoptimized={String(product.thumbnailUrl || '').includes('via.placeholder.com')}
                />
            </div>
